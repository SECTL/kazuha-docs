...
import { computed, ref, watch, watchEffect, watchPostEffect, onMounted } from 'vue'
...
function containsActiveLink(path: string, items: DefaultTheme.SidebarItem[]) {
  return items.some((item) =>
    isActive(path, 'link' in item ? item.link : undefined) ||
    ('items' in item && item.items ? containsActiveLink(path, item.items) : false)
  )
}

function useSidebarControl(item: ReturnType<typeof computed>) {
